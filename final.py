...
for epoch in range(10):
    for images, labels in dataloader:
        ...
        for i in range(len(o)):
            if(o[i] > 0.5 and labels[i] == 1): correctPreds += 1
            elif(o[i] < 0.5 and labels[i] == 0): correctPreds += 1
    ...
    print("ACC:", correctPreds/len(dataloader.dataset))
    runningLoss = 0
    correctPreds = 0
...
for epoch in range(10):
    for images, labels in dataloader:
        ...
        for i in range(len(outputs)):
            print(outputs[i].cpu().detach().numpy(), labels[i].cpu().detach().numpy())
        break
